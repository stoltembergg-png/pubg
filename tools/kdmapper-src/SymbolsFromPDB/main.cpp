...
int main()
{
	...
	std::vector<SymNeeded> symbolsToRetrieve{
		{ L"ntoskrnl.exe", L"MmAllocateIndependentPages" },
	...
	// Current Windows installations may place WdFilter.sys directly under
	// drivers rather than in a `wd` subdirectory. Prefer the upstream path,
	// then use the actual local layout so WdFilter symbols are not silently
	// omitted from an offsets refresh.
	std::vector<std::wstring> WdFilterPaths = {
		L"C:\\Windows\\System32\\drivers\\wd\\WdFilter.sys",
		L"C:\\Windows\\System32\\drivers\\WdFilter.sys"
	};
	for (const auto& WdFltrPath : WdFilterPaths)
	{
		if (!std::filesystem::exists(WdFltrPath))
			continue;
		targetBinaries.push_back(WdFltrPath);
		symbolsToRetrieve.push_back({ L"WdFilter.sys", L"MpBmDocOpenRules" });
		symbolsToRetrieve.push_back({ L"WdFilter.sys", L"MpFreeDriverInfoEx" });
		break;
	}
