...
uint64_t intel_driver::MmAllocateIndependentPagesEx(uint32_t size)
{
	...
	{
		// The local 26100.9457 PDB exposes this routine as
		// MmAllocateIndependentPages (without the Ex suffix).
		kernel_MmAllocateIndependentPagesEx = KDSymbolsHandler::GetInstance()->GetOffset(L"MmAllocateIndependentPages");
		if (!kernel_MmAllocateIndependentPagesEx)
	...
	if (!kernel_MmAllocateIndependentPagesEx)
	{
		//Upstream tested from 1803 to 24H2; offline-validated on ntoskrnl 10.0.26100.9457.
...
#else
	auto sig = FindPatternInSectionAtKernel("PAGE", ci, PUCHAR("\x48\x8B\x1D\x00\x00\x00\x00\xEB\x00\xF7\x43\x40\x00\x20\x00\x00\x75\x40"), "xxx????x?xxxxxxxxx");
	if (!sig) {
		kdmLog(L"[-] Can't Find g_KernelHashBucketList" << std::endl);
		return false;
	}
	// The short LEA signature is duplicated by adjacent CI cache resources on
	// 26100.9457. Keep the function-local prologue so the intended lock is
	// selected exactly once without hard-coding its RIP displacement.
	auto sig2 = FindPatternAtKernel((uintptr_t)sig - 50, 50,
		PUCHAR("\x44\x8D\x7D\x01\x41\x8A\xD7\x48\x8D\x0D"), "xxxxxxxxxx");
	if (!sig2) {
		kdmLog(L"[-] Can't Find g_HashCacheLock" << std::endl);
		return false;
	}
	sig2 += 7; // skip the local prologue to the LEA instruction
